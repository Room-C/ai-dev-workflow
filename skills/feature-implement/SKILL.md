...
argument-hint: "<module> [--version <version>] [design-path] [--side backend|frontend|mobile|fullstack] [--skip N,M,...] [--start-from N] [--strict-verify]"
...
- **module**: 模块名称（必需）
- **version**: 版本号（可选，但同模块多版本时必填）
- **design_path**: design.md 路径（可选，未提供时搜索 `docs/features/{module}/{version}/design.md`）
- **side**: 变更侧（可选，存在 side-specific design/tasks 时建议显式传入）
...
1. 读取项目规则文件（`AGENTS.md` -> `CLAUDE.md` -> README/Makefile/package 配置）
2. 读取 `references/shared/known-issues.md`，主动规避匹配场景
3. 解析目标设计路径：
   - `design_path` 显式传入时直接使用
   - 否则按 `module + version + side` 搜索
   - 未指定 `version` 且模块只有一个版本目录时，可自动复用
   - 未指定 `version` 且模块有多个版本目录时，停止并要求用户明确指定
   - 未指定 `side` 时，优先根目录 `design.md`；若只有一个 side 子目录存在 `design.md`，可自动选中；若存在多个 side 设计稿，则要求用户明确指定 `--side`
4. 读取 design.md（用户提供路径或按上面的规则解析）
...
读取 `references/agents/decomposer.md`。若宿主支持子代理，派发 decomposer agent；否则主上下文按该 reference inline 拆解。传入 design.md 内容、module 名称、项目目录结构、输出路径。
...
2. **从项目规则文件的 Verification / test / quality 章节读取验证命令**（无此章节则从 package/Makefile/CI 配置推断；仍无命令则提示用户补充）
...
1. 计算当前验证指纹：`branch`、`head_sha`、验证命令列表的 `verification_fingerprint`
2. 若已有 `.baseline-snapshot.json`，先校验是否 < 24 小时，且 `branch + head_sha + verification_fingerprint` 全部匹配；只有全部匹配才允许复用
3. 若不能复用，则逐个运行项目验证命令，捕获输出，统计错误数（stderr 行数 + stdout 中 `error`/`Error`/`ERROR`/`FAILED` 行数去重；pytest 额外解析 `N failed`）
4. 将新结果写入 `.baseline-snapshot.json`（与 tasks.md 同目录）：`{ captured_at, branch, head_sha, verification_fingerprint, baseline_mode, commands: { <id>: { command, exit_code, error_count, failed_tests, output_digest(前20行) } } }`
5. 判定：全部 exit_code==0 且 error_count==0 → `clean`，否则 → `delta`
6. 汇报：clean → `✅ 全量验证模式` / delta → `⚠️ 检测到 N 个预存错误，增量验证模式`
...
#### 派发 task-runner 子代理 / inline fallback

```
使用 references/agents/task-runner.md 作为执行 prompt。
参数：任务详情 + design.md 路径 + 红线 + 项目约定。
```

如果宿主没有子代理能力，主上下文按 `task-runner.md` 的步骤执行复杂任务；不要因为缺少 Agent 工具而中断实现。
...
**验证命令来源**：项目规则文件的 Verification / test / quality 章节，或 package/Makefile/CI 配置，不硬编码。
...
📋 后续可选操作：
   - 运行 rc:commit-pr 提交代码并创建 PR（默认目标分支为 main）
