...
如果提供了 `impl-screenshot`，直接使用。否则根据平台尝试自动截取。

**重要边界**：本 Skill 不承诺通用 UI 导航能力。只有以下场景适合自动截取：

- 应用启动后目标页面就是当前页
- 项目本身已有可重复使用的 deep link / debug route / test hook，可稳定进入目标页
- 用户已手动把模拟器/设备停留在目标页，Skill 只负责截图

若以上条件都不满足，应立即要求用户提供 `impl-screenshot`，或先手动把页面切到目标状态再继续。

**iOS:**
1. 确认模拟器已启动（`boot_sim` 如需要）
2. 构建并运行应用（`build_run_sim`）
3. 若项目具备可重复进入目标页的机制，则进入目标页面；否则停止并要求用户提供截图或手动定位页面
4. 截取模拟器截图（`screenshot`）

**Flutter:**
1. 确认应用已启动（`launch_app` 如需要）
2. 若项目具备可重复进入目标页的机制，则进入目标页面；否则停止并要求用户提供截图或手动定位页面
3. 获取截图
...
3. 重新构建和截图
4. 重新对比验证
5. 仅当截图获取路径可重复时，才循环自动修复；否则输出修复建议并停止
...
6. **不承诺通用导航** — 没有稳定页面进入机制时，优先要求用户提供截图或手动把页面切到目标态
