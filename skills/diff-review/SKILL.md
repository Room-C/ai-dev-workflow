...
5. **噪音过滤**：预处理脚本已内置（`*.lock`、`*.generated.*`、`*/migrations/*`、`*.min.*`、`*.pbxproj`）
...
1. Codex Skill → 2. Codex Bash 直调 → 3. `feature-dev:code-reviewer` 子 agent → 4. 原生内联审查（仅用 Read/Grep/git）
...
3. 在 CLAUDE.md / AGENTS.md 相应 section 追加一行规则引用
4. 在最终报告末尾追加"知识沉淀"摘要
