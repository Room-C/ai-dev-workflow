...
| 与项目约定冲突但项目明文采纳 | "不应使用 any"，但 AGENTS.md / CLAUDE.md 明确允许 | dismissed，evidence 引用约定 |
...
| 2 | **建议引入项目拒绝的库** | "用 Alamofire 简化"、"用 axios 替代 fetch"、"用 lodash 替代手写" | 项目若 `package.json` / `Package.swift` 无该依赖且 AGENTS.md / CLAUDE.md 标注了 tech stack → dismissed |
...
| 5 | **测试覆盖度抱怨** | "这个函数缺少测试"——但项目策略免测（prototype、migration、脚手架） | 查 AGENTS.md / CLAUDE.md testing policy；无规则时默认 P3 `deferred` 而非 `dismissed` |
...
| 8 | **对手工维护的配置/构建文件建议自动化** | "用 XcodeGen 生成 pbxproj"、"用工具管理 Cargo.toml" | AGENTS.md / CLAUDE.md 若声明手动管理 → dismissed |
