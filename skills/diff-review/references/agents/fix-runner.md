...
从一开始就维护**本条 fix 的显式文件边界**：

- 默认 `FILES_TOUCHED=("$FILE")`
- 只有当 finding 明确要求同步修改第二个文件时，才把该文件追加进去
- **禁止**用 repo 级 `git diff --name-only` 推断本条边界；那会把其他未提交改动混进来
...
记录 `files_touched` 时只看本条 fix 的显式文件集合：

```bash
TRACKED_TOUCHED=$(git diff --name-only -- "${FILES_TOUCHED[@]}" 2>/dev/null || true)
UNTRACKED_TOUCHED=$(git ls-files --others --exclude-standard -- "${FILES_TOUCHED[@]}" 2>/dev/null || true)
FILES_JSON=$(printf '%s\n%s\n' "$TRACKED_TOUCHED" "$UNTRACKED_TOUCHED" \
  | awk 'NF' | sort -u | jq -R -s 'split("\n") | map(select(length > 0))')
```

如果某个候选文件最终没有被修改，不要把它写进 `files_touched`。
...
if [ -z "$LINT$TYPECHECK$TEST" ]; then
  ...
  FAILED_KINDS=()  # 失败的验证类型集合：lint / typecheck / test

  for KIND in lint typecheck test; do
    case "$KIND" in
      lint)      CMD="$LINT" ;;
    ...
    # 每类验证独立 log，避免 lint→typecheck→test 顺序覆写导致前序结果丢失
    LOG="$OUTPUT_DIR/fix-$ROUND-$ID-$KIND.log"
    set +e
    eval "$CMD" > "$LOG" 2>&1
    RC=$?
    set -e
  ...
  [ ${#FAILED_KINDS[@]} -eq 0 ] && VERIFICATION="passed"
fi
```

**关键**：验证日志按 `$KIND` 分文件写入 `$OUTPUT_DIR`，**不回传给主 Skill**。只把结构化结论放入 JSON。
...
仅对 `FAILED_KINDS` 中的类型逐一做 set-diff：

```bash
ALL_NEW=()
PRE_EXISTING_OBSERVED=()

for KIND in "${FAILED_KINDS[@]}"; do
  LOG="$OUTPUT_DIR/fix-$ROUND-$ID-$KIND.log"

  # 当前 fix 后该类的失败行
  CURRENT_FAILS=$(grep -v '^$' "$LOG" | sort -u)

  # baseline 已有该类失败（baseline.missing=true 时视为空集，全部计入新增）
  if [ "$MISSING" = "true" ]; then
    BASELINE_FAILS=""
  else
    BASELINE_FAILS=$(jq -r ".failures.$KIND[]?" "$BASELINE_JSON" | sort -u)
  fi

  # 差集 = 本轮新增
  NEW_FAILS=$(comm -23 <(echo "$CURRENT_FAILS") <(echo "$BASELINE_FAILS"))

  if [ -n "$NEW_FAILS" ]; then
    ALL_NEW+=("$KIND: $NEW_FAILS")
  else
    PRE_EXISTING_OBSERVED+=("$KIND: $CURRENT_FAILS")
  fi
done
...
if [ ${#ALL_NEW[@]} -gt 0 ]; then
  ...
  VERIFICATION="failed-pre-existing"
fi
```
...
- `verification=failed-new` → 回滚修复：
  ```bash
  TRACKED_TO_RESTORE=$(git diff --name-only -- "${FILES_TOUCHED[@]}" 2>/dev/null || true)
  [ -n "$TRACKED_TO_RESTORE" ] && git checkout -- $TRACKED_TO_RESTORE
  UNTRACKED_TO_REMOVE=$(git ls-files --others --exclude-standard -- "${FILES_TOUCHED[@]}" 2>/dev/null || true)
  [ -n "$UNTRACKED_TO_REMOVE" ] && rm -f -- $UNTRACKED_TO_REMOVE
  ```
...
1. **工作树一致性**：任何异常路径（edit 失败 / 验证异常 / 脚本崩溃）都必须确保工作树无悬空改动。失败回滚只允许触达本条 `FILES_TOUCHED`，tracked 用 `git checkout --`，untracked 用 `rm -f`。
...
7. **files_touched 严格对应本条 fix**：绝不使用 repo 级 `git diff --name-only`。只列本条显式编辑过且确实产生 diff 的文件，防止误把之前的未提交改动记入。
