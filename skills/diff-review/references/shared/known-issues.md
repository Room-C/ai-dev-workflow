...
- **[2026-05-21]** 预处理脚本路径必须优先解析当前 Skill 目录内的 `scripts/preprocess-diff.sh`。Claude plugin cache 只能作为 legacy fallback，不能作为通用 Agent Skills 包的主路径。
- **[2026-05-21]** Host-native review / Codex CLI 可能不可用或不支持 focus/path 过滤。必须降级到 bundled `diff-reviewer` 的 agent-fallback 或 native-inline，不得把外部 review 引擎当成硬依赖。
- **[2026-05-21]** 不得为启用 nested review 去 patch 宿主插件文件或全局配置；这会破坏通用包语义。宿主不支持时直接跳过 L1/L2。
