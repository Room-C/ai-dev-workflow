...
Codex should translate user input into a call to the bundled script:

```bash
scripts/branch-create.sh <newBranch>
scripts/branch-create.sh <originBranch> <newBranch>
```

The script itself does not parse `rc:branch create`; that prefix is the semantic command users type in Codex.
...
```bash
scripts/branch-create.sh --delete-existing-local <originBranch> <newBranch>
```
